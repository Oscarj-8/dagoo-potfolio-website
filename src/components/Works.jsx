...
function Works() {
  ...
  const filterWorksByCategory = (category) => {
  ...
  return (
    ...
    >
      <div className="mt-[550px] flex flex-col items-center gap-8 z-10 md:mt-[500px] lg:mt-[400px] xl:mt-[300px] 2xl:mt-[150px] xl:gap-16 w-[100vw] mb-20">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 text-white rounded-md">
          <span
            className={`cursor-pointer p-1 backdrop-blur-sm border border-transparent transition-all duration-300 ease-in-out rounded-md ${
              selectedCategory === null ? "text-animation catagory" : ""
            }`}
          ...
          <span
            className={`p-2 cursor-pointer  backdrop-blur-sm border border-transparent transition-all duration-300 ease-in-out rounded-md ${
              selectedCategory === "Graphics Design"
                ? "text-animation catagory"
                : ""
            }`}
            onClick={() => filterWorksByCategory("Graphics Design")}
          >
            Graphics Design
          </span>
          <span
            ...
            onClick={() => filterWorksByCategory("Photography")}
          >
            Photography
          </span>
          <span
            className={`cursor-pointer p-2   backdrop-blur-sm border border-transparent transition-all duration-300 ease-in-out rounded-md ${
              selectedCategory === "Social media management"
                ? "text-animation catagory"
                : ""
            }`}
            onClick={() => filterWorksByCategory("Social media management")}
          >
            Social media management
          </span>
        </div>
        <div className="flex flex-wrap gap-14 justify-center z-10">
          {selectedCategory
            ? works
                .filter((work) => work.catagory === selectedCategory)
                .map((filteredWork) => (
                  <div
                    key={filteredWork.id}
                    className="card  transition-all duration-500 ease-in-out cursor-pointer hover:shadow-2xl flex flex-col w-80  rounded-2xl h-[28em] overflow-hidden"
                  >
                    <div className="bg-gradient-to-r from-secondaryColor to-cardHeaderMain rounded-t-2xl p-3 text-white">
                      <h1 className="font-medium text-lg">
                        {filteredWork.workType}
                      </h1>
                      <p className="font-light">{filteredWork.client}</p>
                    </div>
                    <div className="h-full overflow-hidden">
                      <img
                        loading="lazy"
